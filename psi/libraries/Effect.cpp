...
std::string StatToString(const StatType statType)
{
	switch (statType)
	{
...
{// target - card(unit. item, spell)
	...
	description += "buffs " + StatToString(statType) + " by " + std::to_string(value) + ", ";
	description += ", targets ";
...
{// target - card(unit) or hero
	...
	description += "restores " + std::to_string(value) + " health, ";
...
{// target - card(unit) or hero
	...
	description += "deals " + std::to_string(value) + " damage, ";
...
{// target - card(unit)
	...
	description += "applies \"" + StatusToString(status) + "\", ";
...
{// target - card(unit)
	...
	description += "adds keyword \"" + KeywordToString(keyword) + "\", ";
	switch (durationType) {
	case EffectDuration::INSTANT: description += "effect is applied instantly. "; break;
	case EffectDuration::PERMANENT: description += "effect lasts forever. "; break;
	case EffectDuration::TURN_BASED: description += "effect applies every turn for " + std::to_string(numberOfTurns.value()) + " turns. "; break;
...
{// target - card(unit)
	...
	description += "removes \"" + StatusToString(status) + "\", ";
	description += " Targets ";
	switch (targetMode) {
	case TargetMode::SELF: description += "this unit"; break;
	case TargetMode::RANDOM_SINGLE: description += "a random single unit"; break;
...
{// target - card(unit, item, spell) to steal from opponent
	...
	description += "steals " + std::to_string(numberOfCards) + " cards from opponents ";
...
{// target - hero, modifies energy of player
	...
	description += "adds " + std::to_string(value) + " to max energy.";
}
